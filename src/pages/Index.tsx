...
const IndexContent = () => {
  ...
  const SectionHeader = ({ title, count }: { title: string; count: number }) => (
    <div className="sticky top-0 bg-white/90 backdrop-blur-sm border-b border-gray-200 py-4 mb-6 z-10">
      <h2 className="text-xl md:text-2xl font-bold text-gray-900 flex items-center gap-2">
        {title === 'Videos' && <Play className="h-5 w-5 md:h-6 md:w-6 text-red-600" />}
        {title === 'Blogs' && <FileText className="h-5 w-5 md:h-6 md:w-6 text-blue-600" />}
        {title === 'Websites' && <Globe className="h-5 w-5 md:h-6 md:w-6 text-green-600" />}
        {title}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      {/* Mobile Header */}
      {isMobile ? (
        <MobileHeader onSignIn={() => setShowAuthModal(true)} />
      ) : (
        /* Desktop Header */
            ...
            <div className="text-center mb-8 md:mb-12">
              ...
              <div className="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
                <Input
                  ...
                  className="text-base md:text-lg py-4 md:py-6 border-2 border-gray-200 focus:border-blue-500 rounded-xl"
                />
                <Button
                  onClick={handleSearchClick}
                  disabled={isSearching}
                  className="px-6 md:px-8 py-4 md:py-6 text-base md:text-lg bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 rounded-xl whitespace-nowrap"
                >
                  {isSearching ? (
                    <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-8 md:mb-12">
                    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-shadow">
                      <div className="w-12 h-12 md:w-16 md:h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <Play className="h-6 w-6 md:h-8 md:w-8 text-red-600" />
                      </div>
                      <h3 className="text-lg md:text-xl font-semibold mb-2">AI-Filtered Videos</h3>
                      <p className="text-gray-600 text-sm md:text-base">Curated educational videos with AI-powered relevance scoring</p>
                    </div>
                    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-shadow">
                      <div className="w-12 h-12 md:w-16 md:h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <Globe className="h-6 w-6 md:h-8 md:w-8 text-green-600" />
                      </div>
                      <h3 className="text-lg md:text-xl font-semibold mb-2">Learning Platforms</h3>
                      <p className="text-gray-600 text-sm md:text-base">Interactive courses from leading educational platforms</p>
                    </div>
                    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-shadow">
                      ...
                      <h3 className="text-lg md:text-xl font-semibold mb-2">Quality Articles</h3>
                      <p className="text-gray-600 text-sm md:text-base">In-depth articles from trusted educational sources</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Recommendations Section - only show after search results */}
      {searchResults && (
