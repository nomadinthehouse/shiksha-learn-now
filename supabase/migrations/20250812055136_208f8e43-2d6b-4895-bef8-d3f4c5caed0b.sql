...
CREATE TABLE IF NOT EXISTS public.user_search_summaries (
...
CREATE UNIQUE INDEX IF NOT EXISTS user_search_summaries_unique
...
CREATE POLICY IF NOT EXISTS "Users can view their own search summaries"
ON public.user_search_summaries
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own search summaries"
ON public.user_search_summaries
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own search summaries"
ON public.user_search_summaries
FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own search summaries"
ON public.user_search_summaries
FOR DELETE
USING (auth.uid() = user_id);
